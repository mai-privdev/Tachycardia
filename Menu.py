...
class Menu:
    def __init__(self, width, height, font, titlefont, title, screen):
        self.width = width
        self.height = height
        self.font = font
        self.titlefont = font
    ...
    def buttonhighlight(self):
        ...
        for i in range(len(Value)):
            if (i + 1)%2 == 0:
                i = (i + 1)%2
                pygame.draw.rect(self.screen,self.buttoncolour,[self.width*0.25,self.height*(0.4+(i*0.2)),200,40])
            elif (i)%2 == 0:
                i = (i)%2
                pygame.draw.rect(self.screen,self.highlightcolour,[self.width*0.25,self.height*(0.4+(i*0.2)),200,40])
            ...
            for event in pygame.event.get():
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if self.height*0.8 <= mouse[1] <= self.height*0.9:
                            MenuActive = False
                            pygame.quit()
        elif not(self.height*0.4 <= mouse[1] <= self.height*0.5):
            self.buttonhighlighcount.append(1)
            self.buttonhighlighcount.append(3)
            self.buttonhighlighcount.append(5)
        #Title
